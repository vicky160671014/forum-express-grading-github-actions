  ...
  getRestaurants: (req, res, next) => {
    ...
    return Promise.all([
      ...
      .then(([restaurants, categories]) => {
        const favoritedRestaurantsId = req.user && req.user.FavoritedRestaurants.map(fr => fr.id) // req.user有可能為空，故寫成&&先做檢查；資料來源為運用passport
        const LikedRestaurantId = req.user && req.user.LikeRestaurants.map(lr => lr.id)
